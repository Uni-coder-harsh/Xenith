...
void printUsage() {
    ...
    std::cout << "  xenith_mps --solve <path-to-mps-file>    " 
              << TerminalUI::colorize("(Solve LP model using Revised Simplex)", ColorScheme::GRAY) << "\n";
    std::cout << "  xenith_mps --solve --verbose <path-to-mps-file> " 
              << TerminalUI::colorize("(Solve with live pivot trace)", ColorScheme::GRAY) << "\n\n";
}
    ...
    try {
        ...
        if (!solve_mode) {
            ...
            if (result.status == xenith::ModelStatus::OPTIMAL) {
                ...
                std::cout << "\n";
                TerminalUI::printBox("✦ LP SOLVER OPTIMAL RESULT DASHBOARD", items, ColorScheme::NEON_GREEN);
                ...
                std::cout << "\n";
                TerminalUI::printBox("✦ LP SOLVER TERMINATION DASHBOARD", items, ColorScheme::RED);
